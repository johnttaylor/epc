...
def run(argv):
    ...
    # setup excludes 
    excludes = '--exclude=.*/Eros/.* --exclude=.*_0test.* --exclude=.*/xsrc/.* --exclude=.*src/Catch.* --exclude=.*src/Json/Arduino.h --exclude=.*src/Json/ArduinoHelpers.cpp --exclude=.*src/Cpl/Json/Ardunio.h --exclude=.*src/Cpl/Type/enum.h --exclude-unreachable-branches --exclude-lines-by-pattern .*CPL_SYSTEM_TRACE.* --exclude-lines-by-pattern .*CPL_SYSTEM_ASSERT.*'
