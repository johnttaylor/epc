...
def run(argv):
    ...
    # setup excludes 
    excludes = '--exclude=.*_0test.* --exclude=.*src/Catch.*  --exclude=.*src/Cpl/Json/Ardunio.h --exclude=.*src/Cpl/Type/enum.h --exclude-unreachable-branches --exclude-lines-by-pattern .*CPL_SYSTEM_TRACE.* --exclude-lines-by-pattern .*CPL_SYSTEM_ASSERT.*'
    ...
    if (args['rpt']):
        python = 'python'
        cmd  = '{} -m gcovr {} {} --gcov-ignore-parse-errors=negative_hits.warn -j 8 -r {}/src --object-directory . {} .'.format(python, excludes, arcopt, pkg,  ' '.join(args['<args>']) if args['<args>'] else '') 
        if (args['<args>']):
            first = args['<args>'][0]
